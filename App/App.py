...
def run():
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align='justify'>
            A tool which parses information from a resume and finds the keywords, cluster them onto sectors based on their keywords. And lastly show recommendations, predictions, analytics to the applicant based on keyword matching.
        </p>
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin':
                ...
                cursor.execute('''SELECT ID, resume_score, convert(Predicted_Field using utf8), convert(User_level using utf8) FROM user_data''')
                datanalys = cursor.fetchall()
                plot_data = pd.DataFrame(datanalys, columns=['ID', 'Resume Score', 'Predicted Field', 'User Level'])

                ### Total Users Count with a Welcome Message
                values = plot_data['ID'].count()
                st.success("Total %d Users Have Used Our Tool : )" % values)

                ### Fetch user data from user_data(table) and convert it into dataframe
                cursor.execute('''SELECT ID, act_name, act_mail, act_mob, convert(Predicted_Field using utf8), Timestamp, resume_score, convert(User_level using utf8), convert(Actual_skills using utf8), convert(Recommended_skills using utf8), convert(Recommended_courses using utf8) FROM user_data''')
                data = cursor.fetchall()

                st.header("**User's Data**")
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Mail', 'Mobile Number', 'Predicted Field', 'Timestamp', 'Resume Score', 'User Level', 'Actual Skills', 'Recommended Skills', 'Recommended Course'])
                ...
                # fetching Predicted Field from the query and getting the unique values and total value count                 
